...
public partial class NinjaGameManagerModel {
    ...
    [RealtimeProperty(3, true, true)] int _winnerId = -1;
}
...
public partial class NinjaGameManagerModel : IModel {
    ...
    public int winnerId {
        get { return _cache.LookForValueInCache(_winnerId, entry => entry.winnerIdSet, entry => entry.winnerId); }
        set { if (value == winnerId) return; _cache.UpdateLocalCache(entry => { entry.winnerIdSet = true; entry.winnerId = value; return entry; }); FireWinnerIdDidChange(value); }
    }
    ...
    public delegate void WinnerIdDidChange(NinjaGameManagerModel model, int value);
    public event         WinnerIdDidChange winnerIdDidChange;
    ...
    public void FireWinnerIdDidChange(int value) {
        try {
            if (winnerIdDidChange != null)
                winnerIdDidChange(this, value);
        } catch (System.Exception exception) {
            Debug.LogException(exception);
        }
    }
    ...
    public int WriteLength(StreamContext context) {
        ...
        if (context.fullModel) {
            ...
            length += WriteStream.WriteVarint32Length((uint)PropertyID.WinnerId, (uint)_winnerId);
        } else {
            // Reliable properties
            if (context.reliableChannel) {
                ...
                if (entry.startTimeSet)
                    length += WriteStream.WriteFloatLength((uint)PropertyID.StartTime);
                if (entry.winnerIdSet)
                    length += WriteStream.WriteVarint32Length((uint)PropertyID.WinnerId, (uint)entry.winnerId);
            }
        }
    ...
    public void Write(WriteStream stream, StreamContext context) {
        if (context.fullModel) {
            ...
            stream.WriteVarint32((uint)PropertyID.WinnerId, (uint)_winnerId);
        } else {
            // Reliable properties
            if (context.reliableChannel) {
                LocalCacheEntry entry = _cache.localCache;
                if (entry.gameStateSet || entry.startTimeSet || entry.winnerIdSet)
                ...
                if (entry.winnerIdSet)
                    stream.WriteVarint32((uint)PropertyID.WinnerId, (uint)entry.winnerId);
            }
        }
    }
    ...
    public void Read(ReadStream stream, StreamContext context) {
        ...
        bool winnerIdExistsInChangeCache = _cache.ValueExistsInCache(entry => entry.winnerIdSet);
        ...
        while (stream.ReadNextPropertyID(out propertyID)) {
            switch (propertyID) {
                ...
                case (uint)PropertyID.WinnerId: {
                    int previousValue = _winnerId;
                    
                    _winnerId = (int)stream.ReadVarint32();
                    
                    if (!winnerIdExistsInChangeCache && _winnerId != previousValue)
                        FireWinnerIdDidChange(_winnerId);
                    break;
                }
                default:
                    stream.SkipProperty();
